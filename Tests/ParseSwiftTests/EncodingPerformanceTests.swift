...
internal class _JSONEncoder : Encoder {
    ...
    internal let options: OldParseEncoder._Options
    ...
    fileprivate init(options: OldParseEncoder._Options, codingPath: [CodingKey] = []) {
...
let forbiddenKeys = Set(["objectId", "createdAt", "updatedAt", "ACL"])

class EncodingPerformanceTests: XCTestCase {
    func test_encodePerformance() {
        let gameScore = GameScore(score: 12345)
        let encoder = ParseCoding.parseEncoder()
    ...
    func test_oldEncodePerformance() {
        let gameScore = GameScore(score: 12345)
        let encoder = OldParseEncoder()
        encoder.dateEncodingStrategy = .custom(ParseCoding.parseDateEncodingStrategy)
